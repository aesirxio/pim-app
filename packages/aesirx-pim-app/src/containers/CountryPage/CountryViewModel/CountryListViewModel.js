...
class CountryListViewModel {
  ...
  initializeAllData = async () => {
    ...
    const data = await this.countryStore.getListWithoutPagination(this.filter);

    runInAction(() => {
      if (!data?.error) {
        this.callbackOnSuccessGetCountrysHandler(data?.response);
      } else {
        this.onErrorHandler(data?.response);
      }
      this.successResponse.state = true;
  ...
  onSuccessHandler = (result, message) => {
    ...
    if (result?.listItems) {
      this.successResponse.listCountrys = this.transform(result?.listItems);
  ...
  transform = (data) => {
    return data?.map((o) => {
      const date = moment(o[PIM_COUNTRY_DETAIL_FIELD_KEY.MODIFIED_TIME]).format('DD MMM, YYYY');
      return {
        country: {
          id: o[PIM_COUNTRY_DETAIL_FIELD_KEY.ID],
          name: o[PIM_COUNTRY_DETAIL_FIELD_KEY.NAME],
          // level: o[PIM_COUNTRY_DETAIL_FIELD_KEY.LEVEL],
        },
        // countryParent: o[PIM_COUNTRY_DETAIL_FIELD_KEY.PARENT_NAME]
        //   ? o[PIM_COUNTRY_DETAIL_FIELD_KEY.PARENT_NAME]
        //   : 'ROOT',
        lastModified: {
          status: o[PIM_COUNTRY_DETAIL_FIELD_KEY.PUBLISHED],
          dateTime: date ?? '',
          author: o[PIM_COUNTRY_DETAIL_FIELD_KEY.CREATED_USER_NAME],
        },
        published: {
          state: o[PIM_COUNTRY_DETAIL_FIELD_KEY.PUBLISHED],
          id: o[PIM_COUNTRY_DETAIL_FIELD_KEY.ID],
        },
      };
    });
  ...
  deleteCountrys = async (arr) => {
    const data = await this.countryStore.delete(arr);
    runInAction(async () => {
      if (!data?.error) {
  ...
  setPublished = async ({ id, name }, state = 0) => {
    const data = await this.countryStore.update({
      id: id.toString(),
      country_name: name,
  ...
  callbackOnSuccessGetCountrysHandler = (result) => {
    this.successResponse.listCountrysWithoutPagination = result?.listItems?.map((o) => {
      let dash = '';
      for (let index = 1; index < o.level; index++) {
        dash += '- ';
      }
      return { value: o?.id, label: `${dash}${o[PIM_COUNTRY_DETAIL_FIELD_KEY.NAME]}` };
    });
  };
