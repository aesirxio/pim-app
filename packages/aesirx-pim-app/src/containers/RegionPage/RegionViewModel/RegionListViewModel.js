...
class RegionListViewModel {
  ...
  initializeAllData = async () => {
    ...
    const data = await this.regionStore.getListWithoutPagination(this.filter);

    runInAction(() => {
      if (!data?.error) {
        this.callbackOnSuccessGetRegionsHandler(data?.response);
      } else {
        this.onErrorHandler(data?.response);
      }
      this.successResponse.state = true;
  ...
  onSuccessHandler = (result, message) => {
    ...
    if (result?.listItems) {
      this.successResponse.listRegions = this.transform(result?.listItems);
  ...
  transform = (data) => {
    return data?.map((o) => {
      const date = moment(o[PIM_REGION_DETAIL_FIELD_KEY.MODIFIED_TIME]).format('DD MMM, YYYY');
      return {
        region: {
          id: o[PIM_REGION_DETAIL_FIELD_KEY.ID],
          name: o[PIM_REGION_DETAIL_FIELD_KEY.NAME],
          // level: o[PIM_REGION_DETAIL_FIELD_KEY.LEVEL],
        },
        // regionParent: o[PIM_REGION_DETAIL_FIELD_KEY.PARENT_NAME]
        //   ? o[PIM_REGION_DETAIL_FIELD_KEY.PARENT_NAME]
        //   : 'ROOT',
        lastModified: {
          status: o[PIM_REGION_DETAIL_FIELD_KEY.PUBLISHED],
          dateTime: date ?? '',
          author: o[PIM_REGION_DETAIL_FIELD_KEY.CREATED_USER_NAME],
        },
        published: {
          state: o[PIM_REGION_DETAIL_FIELD_KEY.PUBLISHED],
          id: o[PIM_REGION_DETAIL_FIELD_KEY.ID],
        },
      };
    });
  ...
  deleteRegions = async (arr) => {
    const data = await this.regionStore.delete(arr);
    runInAction(async () => {
      if (!data?.error) {
  ...
  setPublished = async ({ id, name }, state = 0) => {
    const data = await this.regionStore.update({
      id: id.toString(),
      region_name: name,
  ...
  callbackOnSuccessGetRegionsHandler = (result) => {
    this.successResponse.listRegionsWithoutPagination = result?.listItems?.map((o) => {
      let dash = '';
      for (let index = 1; index < o.level; index++) {
        dash += '- ';
      }
      return { value: o?.id, label: `${dash}${o[PIM_REGION_DETAIL_FIELD_KEY.NAME]}` };
    });
  };
...
export default RegionListViewModel;
