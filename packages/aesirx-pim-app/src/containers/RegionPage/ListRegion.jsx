...
import { withRegionViewModel } from './RegionViewModel/RegionViewModelContextProvider';
import { historyPush } from 'routes/routes';

const ListRegion = observer((props) => {
  const { t } = useTranslation();
  let listSelected = [];
  const viewModel = props.model.regionListViewModel;
  useEffect(() => {
              ...
              <div>
                ...
                {/* {value.level === 1 && value?.organization_id === '0' ? (
                  ...
                  <div className="text-green">
                    <button
                      onClick={() => {
                        historyPush(`/regions/edit/${value.id}`);
                      }}
  ...
  const deleteRegions = () => {
    if (listSelected.length < 1) {
      ...
      viewModel.isLoading();
      viewModel.deleteRegions(listSelected);
    }
  };
  ...
  return (
    <div className="px-3 py-4">
      ...
      <div className="mb-3">
        ...
        >
          <Tab eventKey="default" title={t('txt_all_region')} />
        </Tabs>
      </div>
      ...
      <div className="bg-white rounded">
        {viewModel?.successResponse?.state ? (
          <Table
            ...
            data={viewModel?.successResponse?.listRegions}
...
export default withTranslation()(withRegionViewModel(ListRegion));
