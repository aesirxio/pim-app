...
class ShippingZoneListViewModel {
  ...
  successResponse = {
    ...
    listShippingZones: [],
    pagination: null,
    listShippingZonesWithoutPagination: [],
  };
  ...
  setForm = (shippingZoneListViewModel) => {
    this.shippingZoneListViewModel = shippingZoneListViewModel;
  };
  ...
  initializeAllData = async () => {
    ...
    const data = await this.shippingZoneStore.getListWithoutPagination(
      this.successResponse.filters
    );

    runInAction(() => {
      if (!data?.error) {
        this.callbackOnSuccessGetShippingZonesHandler(data?.response);
      } else {
        this.onErrorHandler(data?.response);
      }
      this.successResponse.state = true;
  ...
  onSuccessHandler = (result, message) => {
    ...
    if (result?.listItems) {
      this.successResponse.listShippingZones = this.transform(result?.listItems);
  ...
  transform = (data) => {
    return data?.map((o) => {
      const date = moment(o[PIM_SHIPPING_ZONE_DETAIL_FIELD_KEY.MODIFIED_TIME]).format(
        'DD MMM, YYYY'
      );
      return {
        shippingZone: {
        ...
        zipEnd: o[PIM_SHIPPING_ZONE_DETAIL_FIELD_KEY.ZIP_END],
        lastModified: {
          status: o[PIM_SHIPPING_ZONE_DETAIL_FIELD_KEY.PUBLISHED],
          dateTime: date ?? '',
          author: o[PIM_SHIPPING_ZONE_DETAIL_FIELD_KEY.CREATED_USER_NAME],
        },
        published: {
          state: o[PIM_SHIPPING_ZONE_DETAIL_FIELD_KEY.PUBLISHED],
          id: o[PIM_SHIPPING_ZONE_DETAIL_FIELD_KEY.ID],
        },
      };
    });
  ...
  deleteShippingZones = async (arr) => {
    const data = await this.shippingZoneStore.delete(arr);
    runInAction(async () => {
      if (!data?.error) {
...
export default ShippingZoneListViewModel;
