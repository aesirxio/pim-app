import { TaxGroupApiService, TaxGroupItemModel } from 'aesirx-lib';

class TaxGroupStore {
  async getList(filters) {
    try {
      const getListAPIService = new TaxGroupApiService();
  ...
  async getDetail(id) {
    ...
    try {
      ...
      if (results) {
        const getDetailInfoAPIService = new TaxGroupApiService();
  ...
  async create(createFieldData) {
    try {
      const convertedUpdateGeneralData =
        TaxGroupItemModel.__transformItemToApiOfCreation(createFieldData);
      let resultOnSave;
      const createOrganizationApiService = new TaxGroupApiService();
  ...
  async update(updateFieldData) {
    try {
      const convertedUpdateGeneralData =
        TaxGroupItemModel.__transformItemToApiOfUpdation(updateFieldData);

      let resultOnSave;
      const updateOrganizationApiService = new TaxGroupApiService();
      // eslint-disable-next-line prefer-const
  ...
  async delete(arr) {
    try {
      const aesirxOrganizationApiService = new TaxGroupApiService();
