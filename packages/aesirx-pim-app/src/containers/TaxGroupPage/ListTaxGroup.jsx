...
const ListTaxGroup = observer((props) => {
  const { t } = useTranslation();
  // let listSelected = [];
  const viewModel = props.model.taxGroupListViewModel;
  useEffect(() => {
              ...
              <div>
                ...
                {/* {value.level === 1 && value?.organization_id === '0' ? (
                  ...
                  <div className="text-green">
                    <button
                      onClick={() => {
                        historyPush(`/taxGroups/edit/${value.id}`);
                      }}
  ...
  // const deleteTaxGroups = () => {
  //   if (listSelected.length < 1) {
  //     notify(t('txt_row_select_error'), 'error');
  //   } else {
  //     viewModel.isLoading();
  //     viewModel.deleteTaxGroups(listSelected);
  //   }
  // };
  ...
  return (
    <div className="px-3 py-4">
      ...
      <div className="mb-3">
        ...
        >
          <Tab eventKey="default" title={t('txt_all_tax_group')} />
        </Tabs>
      </div>
      ...
      <div className="bg-white rounded">
        {viewModel?.successResponse?.state ? (
          <Table
            ...
            data={viewModel?.successResponse?.listTaxGroups}
...
export default withTranslation()(withTaxGroupViewModel(ListTaxGroup));
