...
class CustomerGroupListViewModel {
  ...
  successResponse = {
    ...
    listCustomerGroups: [],
    pagination: null,
    listCustomerGroupsWithoutPagination: [],
  };
  ...
  setForm = (customerGroupListViewModel) => {
    this.customerGroupListViewModel = customerGroupListViewModel;
  };
  ...
  initializeAllData = async () => {
    ...
    const data = await this.customerGroupStore.getListWithoutPagination(
      this.successResponse.filters
    );

    runInAction(() => {
      if (!data?.error) {
        this.callbackOnSuccessGetCustomerGroupsHandler(data?.response);
      } else {
        this.onErrorHandler(data?.response);
      }
      this.successResponse.state = true;
  ...
  onSuccessHandler = (result, message) => {
    ...
    if (result?.listItems) {
      this.successResponse.listCustomerGroups = this.transform(result?.listItems);
  ...
  transform = (data) => {
    return data?.map((o) => {
      const date = moment(o[PIM_CUSTOMER_GROUP_DETAIL_FIELD_KEY.MODIFIED_TIME]).format(
        'DD MMM, YYYY'
      );
      return {
        customerGroup: {
          id: o[PIM_CUSTOMER_GROUP_DETAIL_FIELD_KEY.ID],
          name: o[PIM_CUSTOMER_GROUP_DETAIL_FIELD_KEY.NAME],
          // level: o[PIM_CUSTOMER_GROUP_DETAIL_FIELD_KEY.LEVEL],
        },
        // customerGroupParent: o[PIM_CUSTOMER_GROUP_DETAIL_FIELD_KEY.PARENT_NAME]
        //   ? o[PIM_CUSTOMER_GROUP_DETAIL_FIELD_KEY.PARENT_NAME]
        //   : 'ROOT',
        lastModified: {
          status: o[PIM_CUSTOMER_GROUP_DETAIL_FIELD_KEY.PUBLISHED],
          dateTime: date ?? '',
          author: o[PIM_CUSTOMER_GROUP_DETAIL_FIELD_KEY.CREATED_USER_NAME],
        },
        published: {
          state: o[PIM_CUSTOMER_GROUP_DETAIL_FIELD_KEY.PUBLISHED],
          id: o[PIM_CUSTOMER_GROUP_DETAIL_FIELD_KEY.ID],
        },
      };
    });
  ...
  deleteCustomerGroups = async (arr) => {
    const data = await this.customerGroupStore.delete(arr);
    runInAction(async () => {
      if (!data?.error) {
  ...
  callbackOnSuccessGetCustomerGroupsHandler = (result) => {
    this.successResponse.listCustomerGroupsWithoutPagination = result?.listItems?.map((o) => {
      let dash = '';
      for (let index = 1; index < o.level; index++) {
        dash += '- ';
      }
      return { value: o?.id, label: `${dash}${o[PIM_CUSTOMER_GROUP_DETAIL_FIELD_KEY.NAME]}` };
    });
  };
...
export default CustomerGroupListViewModel;
