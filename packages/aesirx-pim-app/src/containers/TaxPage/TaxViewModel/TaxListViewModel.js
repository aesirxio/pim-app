...
class TaxListViewModel {
  ...
  setForm = (taxListViewModel) => {
    this.taxListViewModel = taxListViewModel;
  };
  ...
  initializeAllData = async () => {
    ...
    const data = await this.taxStore.getListWithoutPagination(this.successResponse.filters);

    runInAction(() => {
      if (!data?.error) {
        this.callbackOnSuccessGetTaxsHandler(data?.response);
      } else {
        this.onErrorHandler(data?.response);
      }
      this.successResponse.state = true;
  ...
  onSuccessHandler = (result, message) => {
    ...
    if (result?.listItems) {
      this.successResponse.listTaxs = this.transform(result?.listItems);
  ...
  transform = (data) => {
    return data?.map((o) => {
      const date = moment(o[PIM_TAX_DETAIL_FIELD_KEY.MODIFIED_TIME]).format('DD MMM, YYYY');
      return {
        tax: {
          id: o[PIM_TAX_DETAIL_FIELD_KEY.ID],
          name: o[PIM_TAX_DETAIL_FIELD_KEY.NAME],
          // level: o[PIM_TAX_DETAIL_FIELD_KEY.LEVEL],
        },
        // taxParent: o[PIM_TAX_DETAIL_FIELD_KEY.PARENT_NAME]
        //   ? o[PIM_TAX_DETAIL_FIELD_KEY.PARENT_NAME]
        //   : 'ROOT',
        lastModified: {
          status: o[PIM_TAX_DETAIL_FIELD_KEY.PUBLISHED],
          dateTime: date ?? '',
          author: o[PIM_TAX_DETAIL_FIELD_KEY.CREATED_USER_NAME],
        },
        published: {
          state: o[PIM_TAX_DETAIL_FIELD_KEY.PUBLISHED],
          id: o[PIM_TAX_DETAIL_FIELD_KEY.ID],
        },
      };
    });
  ...
  deleteTaxs = async (arr) => {
    const data = await this.taxStore.delete(arr);
    runInAction(async () => {
      if (!data?.error) {
  ...
  setPublished = async ({ id, name }, state = 0) => {
    const data = await this.taxStore.update({
      id: id.toString(),
      tax_name: name,
  ...
  callbackOnSuccessGetTaxsHandler = (result) => {
    this.successResponse.listTaxsWithoutPagination = result?.listItems?.map((o) => {
      let dash = '';
      for (let index = 1; index < o.level; index++) {
        dash += '- ';
      }
      return { value: o?.id, label: `${dash}${o[PIM_TAX_DETAIL_FIELD_KEY.NAME]}` };
    });
  };
...
export default TaxListViewModel;
