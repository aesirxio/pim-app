import { TaxApiService, TaxItemModel } from 'aesirx-lib';

class TaxStore {
  async getList(filters) {
    try {
      const getListAPIService = new TaxApiService();
  ...
  async getDetail(id) {
    ...
    try {
      ...
      if (results) {
        const getDetailInfoAPIService = new TaxApiService();
  ...
  async create(createFieldData) {
    try {
      const convertedUpdateGeneralData =
        TaxItemModel.__transformItemToApiOfCreation(createFieldData);
      let resultOnSave;
      const createOrganizationApiService = new TaxApiService();
  ...
  async update(updateFieldData) {
    try {
      const convertedUpdateGeneralData =
        TaxItemModel.__transformItemToApiOfUpdation(updateFieldData);

      let resultOnSave;
      const updateOrganizationApiService = new TaxApiService();
      // eslint-disable-next-line prefer-const
  ...
  async delete(arr) {
    try {
      const aesirxOrganizationApiService = new TaxApiService();
