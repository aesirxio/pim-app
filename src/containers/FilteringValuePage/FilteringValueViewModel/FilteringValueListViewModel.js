...
class FilteringValueListViewModel {
  ...
  successResponse = {
    ...
    listFilteringValues: [],
    pagination: null,
    listFilteringValuesWithoutPagination: [],
  };
  ...
  setForm = (filteringValueListViewModel) => {
    this.filteringValueListViewModel = filteringValueListViewModel;
  };
  ...
  initializeAllData = async () => {
    ...
    runInAction(() => {
      if (!data?.error) {
        this.callbackOnSuccessGetFilteringValuesHandler(data?.response);
      } else {
        this.onErrorHandler(data?.response);
      }
      this.successResponse.state = true;
  ...
  onSuccessHandler = (result, message) => {
    ...
    if (result?.listItems) {
      this.successResponse.listFilteringValues = this.transform(result?.listItems);
    ...
    return data.map((o) => {
      ...
      return {
        ...
        lastModified: {
          ...
          dateTime: date ?? '',
          author: o[PIM_FILTERING_VALUE_DETAIL_FIELD_KEY.CREATED_USER_NAME],
        },
        published: {
          state: o[PIM_FILTERING_VALUE_DETAIL_FIELD_KEY.PUBLISHED],
          id: o[PIM_FILTERING_VALUE_DETAIL_FIELD_KEY.ID],
        },
      };
    });
  ...
  deleteFilteringValues = async (arr) => {
    const data = await this.filteringValueStore.delete(arr);
    runInAction(async () => {
      if (!data?.error) {
...
export default FilteringValueListViewModel;
