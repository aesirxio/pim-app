import { SubTypeApiService, SubTypeItemModel } from 'aesirx-lib';

class SubTypeStore {
  async getList(filters) {
    try {
      const getListAPIService = new SubTypeApiService();
  ...
  async getDetail(id) {
    ...
    try {
      ...
      if (results) {
        const getDetailInfoAPIService = new SubTypeApiService();
  ...
  async create(createFieldData) {
    try {
      const convertedUpdateGeneralData =
        SubTypeItemModel.__transformItemToApiOfCreation(createFieldData);
      let resultOnSave;
      const createOrganizationApiService = new SubTypeApiService();
  ...
  async update(updateFieldData) {
    try {
      const convertedUpdateGeneralData =
        SubTypeItemModel.__transformItemToApiOfUpdation(updateFieldData);

      let resultOnSave;
      const updateOrganizationApiService = new SubTypeApiService();
      // eslint-disable-next-line prefer-const
  ...
  async delete(arr) {
    try {
      const aesirxOrganizationApiService = new SubTypeApiService();
