...
const ListFilteringFieldset = observer((props) => {
  const { t } = useTranslation();
  let listSelected = [];
  const viewModel = props.model.filteringFieldsetListViewModel;
  useEffect(() => {
              ...
              <div>
                ...
                {value.level === 1 && value?.organization_id === '0' ? (
                  ...
                  <div className="text-green">
                    <button
                      onClick={() => {
                        historyPush(`/filtering-fieldset/edit/${value.id}`);
                      }}
  ...
  const deleteFilteringFieldsets = () => {
    if (listSelected.length < 1) {
      notify(t('txt_row_select_error'), 'error');
    } else {
      viewModel.isLoading();
      viewModel.deleteFilteringFieldsets(listSelected);
    }
  };
  ...
  return (
    <div className="px-3 py-4">
        ...
        <ActionsBar
          buttons={[
            ...
            },
            {
              ...
              variant: 'success',
              handle: async () => {
                historyPush('/filtering-fieldset/add');
              },
            },
          ]}
        />
      ...
      <div className="mb-3">
        ...
        >
          <Tab eventKey="default" title={t('txt_all_filtering_fieldset')} />
        </Tabs>
      </div>
      ...
      <div className="bg-white rounded">
        {viewModel?.successResponse?.state ? (
          <Table
            ...
            data={viewModel?.successResponse?.listFilteringFieldsets}
...
export default withTranslation()(withFilteringFieldsetViewModel(ListFilteringFieldset));
